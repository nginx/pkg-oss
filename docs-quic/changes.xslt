...
<xsl:param select="'nginx-quic'" name="pkgname"/>
...
<xsl:template match="changes">
    ...
    <xsl:variable name="ver_t">
        <xsl:choose>
            <xsl:when test="position()=1 and $versionmask='1' and $pkgname='nginx-quic'">%%BASE_VERSION%%</xsl:when>
            <xsl:when test="position()=1 and $versionmask='1' and $pkgname!='nginx-quic'">%%MODULE_VERSION%%</xsl:when>
    ...
    <xsl:variable name="rev_t">
        <xsl:choose>
            <xsl:when test="position()=1 and $versionmask='1' and $pkgname='nginx-quic'">%%BASE_RELEASE%%</xsl:when>
            <xsl:when test="position()=1 and $versionmask='1' and $pkgname!='nginx-quic'">%%MODULE_RELEASE%%</xsl:when>
    ...
    <xsl:choose>
    <xsl:when test="$pkgname='nginx-quic' and $format='generic' and @rev!=1"/>
    <xsl:otherwise>
